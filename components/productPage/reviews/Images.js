  ...
  const handleImages = (e) => {
    ...
    files.forEach((img, i) => {
      if (images.length == 3 || i == 2) {
        setError("A maximum of three images are allowed");
        return;
      }
      if (
        img.type !== "image/jpeg" &&
        ...
        setError(
          `${img.name} format is unsupported. Please use only PNG, JPEG, or WEBP.`
        );
        files = files.filter((item) => item.name !== img.name);
        return;
      } else if (img.size > 1024 * 1024 * 5) {
        setError(`${img.name} is too large. A maximum of 5Mb is allowed.`);
        files = files.filter((item) => item.name !== img.name);
        return;
      } else {
        setError("");
